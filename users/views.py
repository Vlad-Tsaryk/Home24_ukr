    ...
    def render_to_response(self, context, **response_kwargs):
        if self.request.is_ajax():
            ...
            filtered_qs = filtered_qs.values(
                "id", "name", "role__role", "phone", "username", "status"
            )
...
class InviteUser(View, SingleObjectMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        email.subject = "Запрошення в Home 24"
        email.body = (
            f"Вас запрошують підключитися до системи Home 24 з рівнем доступу: {user.role.role}.\nВаш логін:"
            f"{user.username} Щоб дізнатися пароль, зв'яжіться з адміністрацією.\nПосилання для входу: "
            f"http://46.101.216.181/admin/site/login"
        )
        email.to = [user.username]
        if email.send():
            messages.success(
                self.request, f"Запрошення користувача <b>{user.username}</b> надіслано"
            )
...
class CreateUser(AdminPermissionRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    success_message = "Користувач створений успішно"
...
class UpdateUser(AdminPermissionRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    success_url = reverse_lazy("user_list")
    success_message = "Користувач оновлений успішно"
...
class DeleteUser(AdminPermissionRequiredMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if self.object.is_superuser:
            messages.error(request, "Неможливо видалити адміністратора")
        else:
            try:
                ...
                if model_name == "application":
                    messages.error(request, "Майстер має заявки")
                else:
                    messages.error(request, "Користувач є менеджером у транзакціях")
...
class UpdateRoles(AdminPermissionRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            ...
            context["role_formset"] = RoleFormSet(
                queryset=Role.objects.exclude(role=Role.RoleName.DIRECTOR).exclude(
    ...
    def form_valid(self, formset):
        ...
        if formset.is_valid():
            print("valid")
            formset.save()
            messages.success(self.request, "Ролі успішно змінені")
