...
class MessageCreate(AdminPermissionRequiredMixin, CreateView):
        ...
        if self.request.is_ajax():
            ...
            if house_id:
                ...
                if floor_id:
                    ...
                    result["floors"] = list(
                        Floor.objects.filter(house=house_id).values("id", "name")
                    )
                if section_id:
                    apartment_filters["section"] = section_id
                else:
                    result["sections"] = list(
                        Section.objects.filter(house=house_id).values("id", "name")
                    )
                result["apartments"] = list(
                    Apartment.objects.filter(**apartment_filters).values("id", "number")
                )
    ...
    def form_valid(self, form):
        ...
        self.object = form
        messages.success(self.request, "Повідомлення успішно створено")
...
class MessageList(AdminPermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return (
            Message.objects.select_related("house", "section", "floor", "apartment")
    ...
    def render_to_response(self, context, **response_kwargs):
        if self.request.is_ajax():
            ...
            for message in result_list:
                message["receivers"] = filtered_qs.get(
                    id=message["id"]
                ).get_receiver_label
...
class MessageDelete(AdminPermissionRequiredMixin, DeleteView):
    permission_required = "messages"
    model = Message
    success_message = "Повідомлення видалено успішно"
    success_url = reverse_lazy("message_list")
...
class MessageDeleteMany(AdminPermissionRequiredMixin, View):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(self.request, "Повідомлення видалено успішно")
