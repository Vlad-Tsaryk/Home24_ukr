...
class StatisticView(RolePermissionRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['owners_active_count'] = User.objects.filter(role=Role.objects.get(role=Role.RoleName.OWNER),
                                                             status=User.StatusName.ACTIVE).count()
        context['application_in_progress'] = Application.objects \
        ...
        context['transactions_income'] = Transaction.objects.filter(type=True, is_complete=True,
                                                                    date__year=current_date.year) \
            .order_by() \
            .annotate(month=TruncMonth('date')) \
            .values('month') \
            .annotate(sum=Sum('sum'))
        context['transactions_outcome'] = Transaction.objects.filter(type=False, is_complete=True,
                                                                     date__year=current_date.year) \
            .order_by() \
            .annotate(month=TruncMonth('date')) \
            .values('month') \
            .annotate(sum=Sum('sum'))
        receipts = Receipt.objects.filter(date__year=current_date.year, is_complete=True).order_by()
        context['receipts_debt_by_month'] = receipts.exclude(status=Receipt.StatusName.PAID) \
            .annotate(month=TruncMonth('date')) \
            .values('month')\
            .annotate(sum=Sum('total_price'))
        context['receipts_paid_by_month'] = receipts.filter(status=Receipt.StatusName.PAID) \
            .annotate(month=TruncMonth('date')) \
            .values('month') \
            .annotate(sum=Sum('total_price'))
        ...
        for dt in rrule.rrule(rrule.MONTHLY, dtstart=start_day_of_current_year, count=12):
            ...
            if not any(d['month'] == dt.date() for d in context['transactions_outcome']):
            ...
            if not any(d['month'] == dt.date() for d in context['receipts_paid_by_month']):
                context['receipts_paid_by_month'].append({'month': dt.date(), 'sum': 0})
        context['transactions_income'] = sorted(context['transactions_income'], key=lambda d: d['month'])
