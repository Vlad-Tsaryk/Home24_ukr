...
class ApartmentTariffView(OwnerPermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context["apartment_name"] = Apartment.objects.get(
            pk=self.kwargs.get("pk")
        ).info()
        return context

    def get_queryset(self):
        return TariffService.objects.select_related("tariff", "service__unit").filter(
            tariff__apartment=self.kwargs.get("pk")
        )
