...
class ApartmentForm(forms.ModelForm):
    ...
    personal_account_select = forms.ModelChoiceField(widget=forms.Select(attrs={'class': 'form-control'}),
                                                     queryset=PersonalAccount.objects.filter(apartment__isnull=True),
                                                     empty_label='Выберите...', required=False)
    personal_account = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control'}))
        ...
        if instance:
            try:
                self.fields['personal_account'].initial = PersonalAccount.objects.get(
                    apartment_id=instance.pk).number
    ...
    def clean_personal_account(self):
        ...
        if number:
            ...
            if PersonalAccount.objects.filter(number=number, apartment__isnull=False).exists():
                raise ValidationError("Лицевой счет уже используется")
