...
def apartment_house_details(house_id, **response_kwargs):
    ...
    return JsonResponse(
        {"section": section_list, "floor": floor_list}, safe=False, **response_kwargs
    )
...
class ApartmentCreate(AdminPermissionRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    success_message = "Квартира успішно створена"
...
class ApartmentUpdate(AdminPermissionRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    form_class = ApartmentForm
    success_message = "Квартира успішно оновлена"
    success_url = reverse_lazy("apartment_list")
...
def apartment_delete(request, pk):
    ...
    try:
        ...
        messages.error(request, f"Не вдалося видалити квартиру")
    if name:
        messages.success(request, f"Квартиру {name} видалено успішно")
        ...
        if self.request.is_ajax():
            ...
            if self.request.GET.get("house_select"):
                result["sections"] = list(
                    Section.objects.filter(house=filter_fields["house"]).values()
                )
                result["floors"] = list(
                    Floor.objects.filter(house=filter_fields["house"]).values()
                )
            ...
            for apartment in data:
                try:
                    apartment["balance"] = PersonalAccount.objects.get(
