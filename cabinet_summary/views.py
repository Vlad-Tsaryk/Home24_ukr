...
class ApartmentSummary(OwnerPermissionRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        receipts_for_the_year = Receipt.objects.filter(
            personal_account__apartment=self.object,
            is_complete=True,
            date__year=current_date.year,
        ).order_by()

        context["receipts_debt_by_month"] = (
            receipts_for_the_year.annotate(month=TruncMonth("date"))
            .values("month")
            .annotate(sum=Sum("total_price"))
        )
        context["receipts_agv_debt"] = context["receipts_debt_by_month"].aggregate(
            Avg("sum")
        )["sum__avg"]

        context["year_services_outcome"] = (
            receipts_for_the_year.prefetch_related("receiptservice_set")
            .values("receiptservice__service__name")
            .annotate(
                sum=Sum(
                    F("receiptservice__price_unit") * F("receiptservice__consumption")
                )
            )
        )

        context["previous_month_services_outcome"] = (
            Receipt.objects.filter(personal_account__apartment=self.get_object())
            .filter(
                is_complete=True,
                date__year=current_date.year,
                date__month=current_date.month - 1,
            )
            .order_by()
