...
class OwnerCreateForm(UserCreationForm):
    ...
    status = forms.ChoiceField(
        ...
        choices=User.StatusName.choices,
        required=True,
    )
    ...
    notes = forms.CharField(
        widget=forms.Textarea(
            attrs={"class": "form-control", "rows": "10", "style": "height: 256px"}
        ),
        required=False,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.initial.get("uid"):
            try:
                self.initial["uid"] = str(
                    User.get_owners().order_by("-pk").first().pk + 1
                ).zfill(6)
            except:
...
class OwnerInviteForm(forms.Form):
    phone = PhoneNumberField(
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "+380731234567"}
        ),
        required=False,
    )
    phone.error_messages["invalid"] = "Введіть коректний номер телефону."
    email = forms.CharField(
        widget=forms.EmailInput(
            attrs={"class": "form-control", "placeholder": "your_email@gmail.com"}
        )
    )
