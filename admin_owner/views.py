...
class OwnerCreate(AdminPermissionRequiredMixin, CreateView, SuccessMessageMixin):
    ...
    success_message = "Власника квартири успішно створено"
...
class OwnerList(AdminPermissionRequiredMixin, ListView):
    ...
    def render_to_response(self, context, **response_kwargs):
        if self.request.is_ajax():
            ...
            result_list = list(
                filtered_qs.values(
                    "id", "name", "phone", "username", "status", "date_joined", "uid"
                )
            )
            owner_without_debt = []
            for owner in result_list:
                owner["apartment"] = list(
                    filtered_qs.get(pk=owner["id"])
                    .apartment_set.all()
                    .values("id", "number", "house__name", "house_id")
                )
...
class OwnerInvite(AdminPermissionRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        email.subject = "Home24"
        email.body = "Вас запрошують підключитися до системи Home24.\n\
        Завантажте програму:\n\
        Play Market: https://play.google.com/\n\
        App Store: https://itunes.apple.com/\n\
        Для отримання додаткової інформації зв'яжіться з адміністрацією."
