...
def house_delete(request, pk):
    ...
    messages.success(request, f"Будинок {obj_house.name} успішно видалено")
    obj_house.delete()
...
class HouseCreate(AdminPermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            context["section_formset"] = SectionFormSet(
                self.request.POST, prefix="section"
            )
            context["floor_formset"] = FloorFormSet(self.request.POST, prefix="floor")
            context["house_user_formset"] = HouseUserFormSet(
                self.request.POST, prefix="house_user"
            )
        else:
            context["section_formset"] = SectionFormSet(
                prefix="section", queryset=Section.objects.none()
            )
            context["floor_formset"] = FloorFormSet(
                prefix="floor", queryset=Floor.objects.none()
            )
            context["house_user_formset"] = HouseUserFormSet(
                prefix="house_user", queryset=HouseUser.objects.none()
            )
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            ...
            return self.form_valid(
                form, floor_formset, section_formset, house_user_formset
            )
        else:
    ...
    def form_valid(self, form, floor_formset, section_formset, house_user_formset):
        ...
        messages.success(self.request, f"Будинок {house.name} сворено успішно")
...
class HouseUpdate(AdminPermissionRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            ...
            context["floor_formset"] = FloorFormSet(
                prefix="floor", queryset=Floor.objects.filter(house=self.kwargs["pk"])
            )
            context["house_user_formset"] = HouseUserFormSet(
                prefix="house_user",
    ...
    def form_valid(self, form, floor_formset, section_formset, house_user_formset):
        ...
        section_formset.save()
        house_user_formset.save()
        messages.success(self.request, f"Будинок {house.name} успішно обновлено")
        return redirect(self.success_url)
