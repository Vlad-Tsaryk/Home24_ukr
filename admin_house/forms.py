...
SectionFormSet = modelformset_factory(model=Section, form=SectionForm, extra=0, can_delete=True)
...
FloorFormSet = modelformset_factory(model=Floor, form=FloorForm, extra=0, can_delete=True)


class HouseUserForm(forms.ModelForm):
    user = forms.ModelChoiceField(widget=forms.Select(attrs={'class': 'form-control'}),
                                  queryset=User.objects.exclude(role__role=Role.RoleName.OWNER), required=True)
    role = RoleModelChoiceField(widget=forms.Select(attrs={'class': 'form-control', 'disabled': ''}),
                                queryset=User.objects.exclude(role__role=Role.RoleName.OWNER), required=False)
...
HouseUserFormSet = modelformset_factory(model=HouseUser, form=HouseUserForm, extra=0, can_delete=True)
