        ...
        if self.request.is_ajax():
            ...
            if self.request.GET.get("owner_change"):
                if owner_id:
                    ...
                    result["personal_accounts"] = list(
                        PersonalAccount.objects.all().values("id", "number")
                    )
...
class TransactionList(AdminPermissionRequiredMixin, ListView):
    ...
    def to_excel(self, value_list):
        ...
        status_dict = {0: "Не проведено", 1: "Проведено"}
        type_dict = {0: "Витрата", 1: "Надходження"}
    ...
    def render_to_response(self, context, **response_kwargs):
        if self.request.is_ajax():
            ...
            filter_fields = {
                ...
                "personal_account__number__contains": self.request.GET.get(
    ...
    def delete(self, request, *args, **kwargs):
        ...
        try:
            if self.object.delete():
                messages.success(
                    self.request, f"Відомість №{obj_number} успішно видалена"
                )
        except:
            messages.error(request, f"Не вдалося видалити відомість")
