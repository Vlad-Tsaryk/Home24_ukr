...
class TariffCreate(AdminPermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            context["tariff_service_formset"] = TariffServiceFormSet(
                self.request.POST, prefix="tariff_service"
            )
        else:
            context["tariff_service_formset"] = TariffServiceFormSet(
                prefix="tariff_service", queryset=TariffService.objects.none()
            )
    ...
    def form_valid(self, form, formset):
        ...
        formset.save()
        messages.success(self.request, f"Тариф {tariff.name} створено успішно")
...
class TariffUpdate(AdminPermissionRequiredMixin, UpdateView):
    ...
    def form_valid(self, form, formset):
        ...
        for tariff_service in tariff_service_formset:
            tariff_service.tariff = tariff
        formset.save()
        messages.success(self.request, f"Тариф {tariff.name} оновлено успішно")
        return redirect(self.success_url)
...
def delete_tariff(request, pk):
    ...
    messages.success(request, f"Тариф {obj_tariff.name} видалено успішно")
    obj_tariff.delete()
