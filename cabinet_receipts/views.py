    ...
    def get_queryset(self):
        if self.kwargs.get("apartment_id"):
            return Receipt.objects.filter(
                personal_account__apartment_id=self.kwargs.get("apartment_id"),
    ...
    def render_to_response(self, context, **response_kwargs):
        if self.request.is_ajax():
            ...
            filtered_qs = filtered_qs.values(
                "id", "date", "status", "number", "total_price"
            )
...
class ReceiptToPDF(OwnerPermissionRequiredMixin, SingleObjectMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response = ReceiptToExcel(
            receipt_object=self.get_object(),
            response_type="action_download",
            excel_template=ex_tmpl,
        ).get_excel()
        filename = ""
        ...
        if content_disposition:
            ...
            if filename_pos >= 0:
                filename = content_disposition[filename_pos + len("filename=") :].strip(
                    '"'
                )[:-5]
