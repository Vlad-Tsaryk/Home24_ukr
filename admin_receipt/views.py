...
class ReceiptCreate(AdminPermissionRequiredMixin, CreateView):
    ...
    success_message = "створена"

    def get_context_data(self, **kwargs):
        context = super(ReceiptCreate, self).get_context_data(**kwargs)
        if self.request.POST:
            context["receipt_service_formset"] = ReceiptServiceFormSet(
                self.request.POST, prefix="receipt_service"
            )
        else:
            context["receipt_service_formset"] = ReceiptServiceFormSet(
                prefix="receipt_service", queryset=ReceiptService.objects.none()
            )
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        if receipt_service_formset.is_valid() and form.is_valid():
            ...
            return super(ReceiptCreate, self).render_to_response(
    ...
    def form_valid(self, form, formset):
        ...
        success(
            self.request, f"Квитанция {receipt.number} {self.success_message} успішно"
        )
        return super().form_valid(form)

    def get_meters_consumption(self):
        metest = (
            Meter.objects.filter(apartment_id=self.request.GET.get("apartment_id"))
            .order_by(
                "service",
                "status",
                "-pk",
        ...
        for meter in metest_new:
            try:
                value = (
                    meter.value
                    - metest_clarified.filter(service=meter.service).last().value
                )
    ...
    def render_to_response(self, context, **response_kwargs):
        if self.request.is_ajax():
            ...
            if self.request.GET.get("set_tariff_services"):
                if tariff_id:
                    result["tariff_services"] = list(
                        TariffService.objects.filter(tariff=tariff_id).values(
                            "service_id", "price"
                        )
                    )
            ...
            if apartment_id or personal_account:
                if apartment_id:
                    ...
                    apartment_obj = PersonalAccount.objects.get(
                        number=personal_account
                    ).apartment
                    ...
                    result["section"] = list(
                        Section.objects.filter(house_id=apartment_obj.house_id).values()
                    )
                ...
                try:
                    apartment_info["personal_account"] = PersonalAccount.objects.get(
                        apartment=apartment_id
                    ).number
                except:
                    pass
                result["meter_list"] = list(
                    Meter.objects.order_by("-date", "-number")
                    .filter(apartment_id=apartment_obj.id)
                    .values(
                        "number",
                        "status",
                        "date",
                ...
                if section_id:
                    ...
                    result["apartment"] = list(
                        Apartment.objects.filter(
                            house_id=house_id, personalaccount__isnull=False
                        ).values("id", "number")
                    )
                    result["section"] = list(
                        Section.objects.filter(house_id=house_id).values()
                    )
                print(result)
            if not self.request.GET.get("meter_set"):
                result["meter_list"] = list(
                    Meter.objects.order_by("-date").values(
                        "number",
                        "status",
                        "date",
                        "apartment__house__name",
        ...
        for pk in self.request.GET.getlist("selected_receipts[]"):
            ...
            try:
                ...
                if obj_delete.delete():
                    delete_info["success_msg"].append(
                        f"Квитанція №{obj_number} успішно видалена"
                    )
            except:
                delete_info["error_msg"].append(
                    f"Не вдалося видалити квитанцію №{obj_number}"
                )
        return delete_info
            ...
            filtered_qs = (
                ...
                .annotate(
                    apartment_info=Concat(
                        Value("Квартира №"),
                        "personal_account__apartment__number",
                        Value(", "),
                        "personal_account__apartment__house__name",
...
def receipt_delete(request, pk):
    ...
    try:
        ...
        obj_number = obj_delete.number
        if obj_delete.delete():
            success(request, f"Квитанція №{obj_number} успішно видалена")
    except:
        error(request, f"Не вдалося видалити квитанцію №{obj_number}")
