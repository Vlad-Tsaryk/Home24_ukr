...
class MainPageForm(forms.ModelForm):
    ...
    image1 = forms.ImageField(
        ...
        required=False,
        error_messages={
            "invalid_image": "До завантаження підтримуються лише зображення"
        },
    )
    ...
    text = forms.CharField(
        widget=forms.Textarea(
            attrs={"class": "form-control compose-textarea", "rows": "6"}
        ),
        required=False,
    )
...
ServiceBlockFormSet = modelformset_factory(
    ServiceBlock, ServiceBlockForm, extra=0, can_delete=True
)
...
TariffBlockFormSet = modelformset_factory(
    TariffBlock, TariffBlockForm, extra=0, can_delete=True
)
...
class DocumentForm(forms.ModelForm):
    ...
    file = forms.FileField(
        widget=forms.FileInput(attrs={"accept": ".pdf, .jpg"}),
        validators=[
