            ...
            if house_id:
                if section_id:
                    ...
                    result = {
                        "apartment": list(
                            Apartment.objects.filter(
                                house_id=house_id, personalaccount__isnull=True
                            ).values(
                                "id",
                                "number",
                                "owner__first_name",
...
def personal_account_delete(request, pk):
    ...
    try:
        ...
        messages.error(request, f"Не вдалося видалити особовий рахунок")
    if number:
        messages.success(request, f"Особовий рахунок № {number} видалено успішно")
...
class PersonalAccountList(AdminPermissionRequiredMixin, ListView):
    ...
    def render_to_response(self, context, **response_kwargs):
        if self.request.is_ajax():
            ...
            filtered_qs = (
                self.get_queryset()
                .prefetch_related("receipt_set", "transaction_set")
                .filter(**filter_fields)
            )
            filtered_qs = filtered_qs.annotate(
                owner__name=Concat(
                    "apartment__owner__first_name",
                    Value(" "),
                    "apartment__owner__middle_name",
                    Value(" "),
            ...
            for personal_account in data:
                personal_account["balance"] = filtered_qs.get(
