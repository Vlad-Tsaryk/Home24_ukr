...
class PersonalAccountBalance:
    ...
    def balance(personal_account):
        try:
            return PersonalAccountBalance.get_total_query(
                filters={"id": personal_account.pk}
    ...
    def get_total_query(filters=None):
        ...
        subquery_transaction = (
            Transaction.objects.filter(
                is_complete=True, personal_account=OuterRef("pk")
            )
            .order_by()
        ...
        subquery_receipt = (
            Receipt.objects.filter(
                Q(is_complete=True, personal_account=OuterRef("pk"))
        ...
        result = PersonalAccount.objects.filter(**filters).annotate(
    ...
    def get_total_balance():
        ...
        return (
            result.filter(balance__gt=0).aggregate(total=Sum("balance"))["total"] or 0
        )
    ...
    def get_total_debt():
        result = PersonalAccountBalance.get_total_query()
        return (
            result.filter(balance__lt=0).aggregate(total=Sum("balance"))["total"] or 0
        )
